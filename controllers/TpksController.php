...
class TpksController extends \app\controllers\base\ContainerController
{
    ...
    public function parseGatein(&$feeds)
    {
        ...
        foreach ($feeds as $feed) {
            $customerId = trim(ArrayHelper::getValue($feed, 'CUSTOMER_ID'));
        ...
        $customers = Customer::find()
            ->where(['ID_CUSTOMER' => array_unique($customerIds)])
            ->indexBy('ID_CUSTOMER')
            ->all();

        foreach ($feeds as $number => $feed) {
            $feeds[$number]['blocked'] = false;
            $customerId = trim(ArrayHelper::getValue($feed, 'CUSTOMER_ID'));

            if ($customerId) {
                $customer = ArrayHelper::getValue($customers, $customerId);
                $feeds[$number]['customer'] = $customer->CUSTOMER;

                if ($customer->BLOCKED == Customer::BLOCKED_YES) {
                    $feeds[$number]['customer'] .= ' (BLOCKED)';
                    $feeds[$number]['blocked'] = true;
                }
            }
        }
        ...
        foreach ($containers as $container) {
            ...
            if ($container->status == Container::STATUS_READY && $verified && $feeds[$number]['blocked'] == false) {
